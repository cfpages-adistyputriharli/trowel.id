...
Source route evidence: sitemap XML, 6 routes discovered during packet generation
Reader community name: `Trowel.id`
...
- Address the reader warmly as `Sobat Trowel.id`, `Kawan Trowel.id`, or `Teman Trowel.id`. Use the exact opening salutation assigned inside the article file.
